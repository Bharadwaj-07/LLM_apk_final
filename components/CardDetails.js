...
const CardDetails = ({ subject, course, instructor, id, fetchClasses, navigation }) => {
    ...
    const findAdmin = async () => {
        ...
        try {
            const response = await axios.post(`http://${GLOBAL_CONFIG.SYSTEM_IP}/api/Attendance/Admin`, {
                course,
                user,
            });
    ...
    const deleteClass = async (classId) => {
        try {
            ...
            const response = await axios.delete(`http://${GLOBAL_CONFIG.SYSTEM_IP}/createClass/${classId}/${userId}/${admin}/${instructor}`);
